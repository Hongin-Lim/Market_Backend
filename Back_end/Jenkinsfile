def DOCKER_IMAGE_NAME = "ddarahakit/msa-search"           // 생성하는 Docker image 이름
            ...
            container('kubectl') {
                withCredentials([usernamePassword(
                    ...
                    passwordVariable: 'PASSWORD')]) {
                        ...
                        sh "echo ${DATE}"
                        sh "sed -i.bak 's#DATE_STRING#${DATE}#' ./django-deployment.yaml"

                        /* yaml파일로 배포를 수행한다 */
                        sh "kubectl apply -f ./django-deployment.yaml -n ${NAMESPACE}"
                        sh "kubectl apply -f ./django-service.yaml -n ${NAMESPACE}"
                }
            }
        }
    }
